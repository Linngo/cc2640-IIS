...
uint8_t Uart_RxTempBuf[100];
uint8_t Uart_TxTempLen;
uint8_t Uart_TxTempBuf[40];
// Uart -> App  Callback
GY_UartRxBufCallback GY_UartReviceDataCallback;


static Clock_Struct BattsClock;
...
#define HID_BATT_EVT           Event_Id_02

#define UARTTASK_EVENT_ALL ( UARTTASK_RX_EVENT | UARTTASK_TX_EVENT | HID_BATT_EVT )
...
void TaskUARTWrite(uint8_t *buf, uint8_t len);  //打印字符串
void TaskUARTPrintf(const char* format, ...);   //printf打印


static void Batts_Evt_Handler(UArg arg);
...
void TaskUART_taskInit(void)
{
  ...
  UART_read(UARTHandle, Uart_RxTempBuf, 100);       //打开一个串口读
  
 // Util_constructClock(&BattsClock, Batts_Evt_Handler,
 //                     60000, 60000, true, HID_BATT_EVT);
 // HwADCInit();
}
...
{ 
  ...
  while(1)
  {
    ...
    {
      UART_read(UARTHandle, Uart_RxTempBuf, 100);     //再次打开一个串口读
    }
...
{ 
  ...
  UART_read(handle, Uart_RxTempBuf, 100);    //再次打开一个串口读
